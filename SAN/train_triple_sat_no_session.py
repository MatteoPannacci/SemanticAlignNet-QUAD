...
from polar_input_data_triple_sat import InputData
...
def train(start_epoch=0):
    ...
    # Full Model
    model = Model(
         inputs=[grdNet.model.input, satNet.model.input, satSegNet.model.input], 
         outputs=[grdNet.model.output, satNet.model.output, satSegNet.model.output]
    )
    ...
    if combination_type == 'concat':
        assert grdNet.out_channels == satNet.out_channels + satSegNet.out_channels
    ...
    grd_features, sat_features, satseg_features = model([grd_x, polar_sat_x, satseg_x])
    # feature extraction and concatenation
    if combination_type == 'concat':
    ...
    # load a Model
    if start_epoch != 0:
        model_path = "./saved_models/triple_sat/12"
    ...
    for epoch in range(start_epoch, start_epoch + number_of_epoch):
        ...
        while True:
            ...
            for i in range(accumulation_size):
                ...
                # take next batch
                batch_sat_polar, batch_sat, batch_grd, batch_satseg, batch_orien = input_data.next_pair_batch(
                    batch_size, 
                ...
                with tf.GradientTape() as tape:
                    ...
                    grd_features, sat_features, satseg_features = model([batch_grd, batch_sat_polar, batch_satseg])
        ...
        while True:
            ...
            # take next batch
            batch_sat_polar, batch_sat, batch_grd, batch_satseg, batch_orien  = input_data.next_batch_scan(
                batch_size, 
