...
class InputData:
    ...
    def next_batch_scan(self, batch_size, grd_noise=360, FOV=360):
        ...
        batch_grddep = np.zeros([batch_size, 128, grd_width, 3], dtype = np.float32)
        batch_sat_polar = np.zeros([batch_size, 128, 512, 3], dtype=np.float32)
        batch_satseg = np.zeros([batch_size, 128, 512, 3], dtype=np.float32)        
        ...
        for i in range(batch_size):
            ...
            # ground depth
            img = cv2.imread(self.img_root + self.id_test_list[img_idx][2].replace("streetview","streetview_depth"))
            img = cv2.resize(img, (512, 128), interpolation=cv2.INTER_AREA)
            img = img.astype(np.float32)
            img = img/255
            img = (img - self.grddep_mean) / self.grddep_std
            j = np.arange(0, 512)
            img_dup = img[:, ((j - random_shift) % 512)[:grd_width], :] # USE SAME SHIFT
            batch_grddep[i, :, :, :] = img_dup
            ###
        ...
        return batch_sat_polar, batch_sat, batch_grd, batch_satseg, batch_grdseg, batch_grddep, batch_orientation
    ...
    def next_pair_batch(self, batch_size, grd_noise=360, FOV=360):
        ...
        while True:
            ...
            ### GROUND DEPTH
            img = cv2.imread(self.img_root + self.id_list[img_idx][2].replace("streetview","streetview_depth"))
            ...
            img_dup = img[:, ((j-random_shift)%512)[:grd_width], :] # use same shift
            batch_grddep[batch_idx, :, :, :] = img_dup
            #######################################
