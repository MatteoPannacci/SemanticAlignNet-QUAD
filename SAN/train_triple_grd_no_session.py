...
from polar_input_data_triple_grd import InputData
...
def train(start_epoch=0):
    ...
    # Full Model
    model = Model(
         inputs=[grdNet.model.input, grdSegNet.model.input, satNet.model.input], 
         outputs=[grdNet.model.output, grdSegNet.model.output, satNet.model.output]
    )
    ...
    if combination_type == 'concat':
        assert grdNet.out_channels + grdSegNet.out_channels == satNet.out_channels
    ...
    grd_features, grdseg_features, sat_features = model([grd_x, grdseg_x, polar_sat_x])
    # feature extraction and concatenation
    if combination_type == 'concat':
    ...
    for epoch in range(start_epoch, start_epoch + number_of_epoch):
        ...
        while True:
            ...
            for i in range(accumulation_size):
                ...
                # take next batch
                batch_sat_polar, batch_sat, batch_grd, batch_grdseg, batch_orien = input_data.next_pair_batch(
                    batch_size, 
                ...
                with tf.GradientTape() as tape:
                    ...
                    grd_features, grdseg_features, sat_features = model([batch_grd, batch_grdseg, batch_sat_polar])
        ...
        while True:
            ...
            # take next batch
            batch_sat_polar, batch_sat, batch_grd, batch_grdseg, batch_orien  = input_data.next_batch_scan(
                batch_size, 
