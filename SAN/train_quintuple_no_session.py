...
from polar_input_data_quintuple import InputData
...
def train(start_epoch=0):
    ...
    grdDepNet = VGGModel(tf.keras.Input(shape=(None, None, 3)),'_grddep', out_channels=8, freeze=True)
    satNet = VGGModelCir(tf.keras.Input(shape=(None, None, 3)),'_sat', out_channels=16, freeze=True)
    satSegNet = VGGModelCir(tf.keras.Input(shape=(None, None, 3)),'satseg', out_channels=16, freeze=True)
 
    # Full Model
    model = Model(
         inputs=[grdNet.model.input, grdSegNet.model.input, grdDepNet.model.input, satNet.model.input, satSegNet.model.input], 
         outputs=[grdNet.model.output, grdSegNet.model.output, grdDepNet.model.output, satNet.model.output, satSegNet.model.output]
    )
    
    print("Model created")

    # The two halves of the model should have the same number of channels in total
    assert grdNet.out_channels + grdSegNet.out_channels + grdDepNet.out_channels == satNet.out_channels + satSegNet.out_channels
    ...
    grddep_x = np.float32(np.zeros([2, 128, width, 3]))
    ...
    grd_features, grdseg_features, grddep_features, sat_features, satseg_features = model([grd_x, grdseg_x, grddep_x, polar_sat_x, satseg_x])
    # feature extraction and concatenation
    ...
    for epoch in range(start_epoch, start_epoch + number_of_epoch):
        ...
        while True:
            ...
            for i in range(accumulation_size):
                ...
                # take next batch
                batch_sat_polar, batch_sat, batch_grd, batch_satseg, batch_grdseg, batch_grddep, batch_orien = input_data.next_pair_batch(
                    batch_size, 
                ...
                with tf.GradientTape() as tape:
                    ...
                    grd_features, grdseg_features, grddep_features, sat_features, satseg_features = model([batch_grd, batch_grdseg, batch_grddep, batch_sat_polar, batch_satseg])
        ...
        while True:
            ...
            # take next batch
            batch_sat_polar, batch_sat, batch_grd, batch_satseg, batch_grdseg, batch_grddep, batch_orien  = input_data.next_batch_scan(
                batch_size, 
